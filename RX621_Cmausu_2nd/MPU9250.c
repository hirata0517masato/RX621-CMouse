...
uint8_t my_low_pass_filter = BITS_DLPF_CFG_188HZ;
uint8_t my_low_pass_filter_acc = BITS_DLPF_CFG_188HZ;
...
unsigned int WriteReg( uint8_t WriteAddr, uint8_t WriteData )
{
    ...
    select();
    rspi_transfer(WriteAddr);
    temp_val=rspi_transfer(WriteData);
    deselect();
...
void ReadRegs( uint8_t ReadAddr, uint8_t *ReadBuf, unsigned int Bytes )
{
    ...
    select();
    rspi_transfer(ReadAddr | READ_FLAG);
    for(i = 0; i < Bytes; i++)
        ReadBuf[i] = rspi_transfer(0x00);
    deselect();
    //delayMicroseconds(50);
}
...
int MPU_init(int calib_gyro, int calib_acc){
    ...
    for(i = 0; i < MPU_InitRegNum; i++) {
        ...
        delay_ITP(1);  // I2C must slow down the write speed, otherwise it won't work
    }
...
void calib_mag(){
    ...
    delay_ITP(50);                                                  // long wait between AK8963 mode changes
...
void calibrate(float *dest1, float *dest2){  
    ...
    delay_ITP(40); // accumulate 40 samples in 40 milliseconds = 480 bytes
