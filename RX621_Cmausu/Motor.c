   ...
   // static int motor_pid_flag_fast = 0;
	...
	if(motor_pid_flag == 0 && w_flag != 3 && w_flag != 2 && kusi_flag == 0){//1msの割り込み内でフラグはリセットされる && 斜めではない && 横壁キャンセルではない &&  串が反応していない
		...
		if(motor_pid_mode == 0){//低速
			...
			}else if(get_IR(IR_L) > 40 && get_IR(IR_R) > 40 ){//左右に壁がある
			...
			}else if(get_IR(IR_LT) > 20 && get_IR(IR_RT) < 15  && get_IR(IR_F) < 50 ){//斜め左だけ壁がある
			...
			}else if(get_IR(IR_LT) < 15 && get_IR(IR_RT) > 20  && get_IR(IR_F) < 50){//斜め右だけ壁がある
...
void ESmotor(long long A, int max_M,char non_stop,char w_flag){
    ...
    static long long enc_base_L = 0;//get_encoder_total_L();
    static long long enc_base_R = 0;//get_encoder_total_R();
    static long long enc_now = 0;
...
//    int kame_hosei;
    ...
    enc_base_R = get_encoder_total_R();
    enc_now = 0;
    ...
    int M_max_safe = 80;//横壁が近すぎる場合は減速
    
    int M_max_safe_fin = 20;//前壁が近すぎる場合は減速
...
*/
				...
				GyroSum_add( (enc_kabe_L - enc_kabe_R) * 2);
			    }
			}else{
/*			    if(Get_motor_pid_mode() == 0){//探索モード
			    	if( non_stop == 0){//迷路探索時
...
*/
	...
	//enc_now = ((get_encoder_total_L()  - enc_base_L) + (get_encoder_total_R() - enc_base_R)) /2;
...
void Tmotor(long long A){
    ...
    static long long L = 0;//get_encoder_total_L();
    static long long R = 0;//get_encoder_total_R();
	
    static long long L_base = 0,R_base = 0;
	...
	/*
	  if(abs(GyroSum_get()) < 100)cnt += 10;
	  else if(abs(GyroSum_get()) < 300)cnt++;
	  else cnt = 0;
...
//    GyroSum_reset();
	...
	while((get_IR(IR_R) > 15) || ( get_IR(IR_F) > 10 && get_IR(IR_F) < 20) ){ //前壁補正は斜めになると悪影響がある
	    Smotor(M_kabe,true);
	   // flag = 1;
	}
...
void ETmotor_search(long long A, long long E, char non_stop){//探索用
    GyroSum_reset();
    //Encoder_reset();
    ...
    if(A > 0){//R 
	//while(get_IR(IR_R) > 10){
	while((get_IR(IR_R) > 10) || ( get_IR(IR_F) > 10 && get_IR(IR_F) < 22) ){ //前壁補正は斜めになると悪影響がある
	    Smotor(M_kabe,true);
	...
	//while(get_IR(IR_L) > 10){
	while((get_IR(IR_L) > 10) || ( get_IR(IR_F) > 10 && get_IR(IR_F) < 22) ){
	    Smotor(M_kabe,true);
	    //Smotor(M_kabe,2);//前壁補正のみ
	    flag = 1;
	}
...
//    GyroSum_reset();
    ...
    */	
	...
	if(A > 0){//R
	    E_offset = rslsr90_search_offset;
	}else{//L
	    E_offset = rslsl90_search_offset;
	}
