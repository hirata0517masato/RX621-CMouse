	   ...
	   if(i == Get_Goal_y() && j == Get_Goal_x()){
		...
		int last = 0;
		 ...
		 while(!queue_empty()){
			...
			if(comand == 0){//直線
				maze_d_perfect[i][j] += max(1,(path_num+1) / 2);
				
			}else if(comand == 10){//斜め直線
				maze_d_perfect[i][j] += max(1,path_num);  //斜め１マス
    ...
    */	
	...
	case -1://L
	    ...
	    */
		...
		L_curve(sl90,true);
		ESmotor(175,30,true,true);//距離、スピード 170
	...
	case 0://S
		...
		if((get_IR(IR_R) > 20 && get_IR(IR_R) < 90 ) || (get_IR(IR_L) > 20 && get_IR(IR_L) < 90 ) || get_IR(IR_L) > 190 || get_IR(IR_R) > 190){
			...
			if(queue_next(1) == -11 || queue_next(1) == 11){//直線後に45ターン
				...
				if(queue_next(1) < 0){//次　左
				    if(path_num <= 1){
					 ...
					 ESmotor(100,40,true,true);//距離、スピード 
					 
					 if(first_flag == 0)first_naname_z = 1;
