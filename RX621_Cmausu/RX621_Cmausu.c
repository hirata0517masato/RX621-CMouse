...
void maze_search_unknown(short* target_x,short* target_y, int Goal_Start){
   ...
   // *target_y = 99;
    ...
    while(x != Goal_x_tmp || y != Goal_y_tmp){
		...
		*target_x = x;//target_x_buf;
	    	*target_y = y;//target_x_buf;
	        return;  
	    }
	    */
    ...
    */	
	...
	case -1://L
	    ...
	    */
		...
		L_curve(sl90,true);
		ESmotor(140,30,true,true);//距離、スピード
	...
	case -11://L45
	    ...
	    break;
	case -14://直線からのL45 
	    if(queue_next(2) <=  1 && queue_next(3) == 0) {//２マス斜め Zパターン
		L_rotate_naname(l45 * path_num * 1.30 ,2);
	
	    }else if(queue_next(2) <=  1 ){//２マス斜め
	...
	case -13://L45 出るとき
		...
		L_rotate_naname(l45 * path_num * 1.10,false);
	...
	case 0://S
		...
		if(queue_next(3) == 0 && queue_next(4) <= 1 && queue_next(5) == 99){//ゴール最後で壁切れが発生しない時（2か所入口があるとき）
			...
			if(queue_next(1) == -11 || queue_next(1) == 11){//直線後に45ターン
				...
				if(queue_next(1) < 0){//次　左
				    if(path_num <= 1){
					 S_run_kabe_BIG(50,4,1); //w_flag = 4 串の壁補正あり
				    }else{
					 S_run_kabe_BIG(40,4,1);  //w_flag = 4 串の壁補正あり 
				    }
						
				}else if(queue_next(1) > 0){//次　右
				    if(path_num <= 1){
					 S_run_kabe_BIG(50,4,2);  //w_flag = 4 串の壁補正あり
				    }else{
					 S_run_kabe_BIG(40,4,2);  //w_flag = 4 串の壁補正あり
				    }
	...
	case 10://Snaname
	    ...
	    if((path_num <= 0 && comand_old != 14 && comand_old != -14 ) || (path_num <= -1 && (comand_old == 14 || comand_old == -14) )){//２マスだけの斜め
		...
		if(queue_next(1) == -11){//次　左
		    ...
		    if(v2_flag == 1 && queue_next(3) != -11){//2マスVターンの出るとき
			 v2_flag = 4; 
	            ...
	            }else if(queue_next(3) == -1){//2マスVターンではない
	...
	case 14://直線からのR45
	    ...
	    if(queue_next(2) <=  1 && queue_next(3) == 0) {//２マス斜め Zパターン
		R_rotate_naname(r45 * path_num * 1.30 ,2);
