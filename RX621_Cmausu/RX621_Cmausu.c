...
*/
 ...
 /*     while(!queue_empty()){//debug
	...
	case -1://L
		...
		ESmotor(50,25,true,true);//距離、スピード
				...
				if(queue_next(1) < 0){//次　左
				    if(path_num <= 1){
					 ...
					 if(first_flag == 0){
						...
						ESmotor(150,20,true,true);//距離、スピード 
					 }
					 
				    }else{
					 S_run_kabe_BIG(20,4,1,path_num);  //w_flag = 4 串の壁補正あり 
					 
					 ESmotor(200,20,true,true);//距離、スピード
				    }
				    ...
				    if(path_num <= 1){
					 ...
					 S_run_kabe_BIG(20,4,2,path_num);  //w_flag = 4 串の壁補正あり
