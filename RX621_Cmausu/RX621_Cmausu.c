...
void S_run_kabe_naname2(int powor, char flag, int LR, int v2_flag){//壁切れまで走行
    ...
    }else if(v2_flag == 21){//２マス Vターンではない
    
    	if(Lflag == 0 && Rflag == 0){
		ESmotor(150,powor,true,false);
	}else{
    		ESmotor(400,powor,true,false);// 200
	}
 ...
 /*     while(!queue_empty()){//debug
	...
	case -1://L
	   ...
	   /* if(comand_old == 0 && queue_next(1) == -1 && queue_next(3) == 0){//Uターン
		...
		//ESmotor(15,25,true,true);//距離、スピード
	...
	case -11://L45
	    if(comand_old == -1){
	    	L_rotate_naname(l45 * path_num * 1.10,true);
	    }else{
	...
	case 10://Snaname
		...
		if(queue_next(1) == -11){//次　左
			...
			else if(comand_old == 11 || comand_old == -11)v2_flag = 21;
		    }else{
		    	v2_flag = 1; 	
		    }	
	...
	case 11://R45
            if(comand_old == 1){
	    	R_rotate_naname(r45 * path_num * 1.10,true);
	    }else{
