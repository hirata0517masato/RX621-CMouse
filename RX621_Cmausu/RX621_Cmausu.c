...
void S_run_kabe_naname2(int powor, char flag, int LR, int v2_flag){//壁切れまで走行
    ...
    if(v2_flag == 1){//２マスVターン直後
	ESmotor(190,powor,true,false); 
    }else if(v2_flag == 3){//２マス以上のVターン直後
    ...
    */	
	...
	case -13://L45 出るとき
	    ...
	    if(queue_next(1) == -11){//Vターン
			...
			L_rotate_naname(l45 * path_num * 2.60,false);//0.75
			GyroSum_reset();
		}
		comand = dequeue();
	      	path_num = dequeue();
	...
	case 10://Snaname
		...
		if(queue_next(1) == -11){//次　左
		    ...
		    if(queue_next(3) != -11){//2マスVターンではない
			S_run_kabe_naname2(55,3,1,2);// v2_flag = 2
		    ...
		    if(queue_next(3) != 11){//2マスVターンではない
			S_run_kabe_naname2(55,3,2,2);// v2_flag = 2
	...
	case 13://R45 出る時
	    ...
	    if(queue_next(1) == 11){//Vターン
			...
			R_rotate_naname(r45 * path_num * 2.60,false);//0.75
			GyroSum_reset();
		}
		comand = dequeue();
	      	path_num = dequeue();
