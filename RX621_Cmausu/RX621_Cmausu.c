	...
	while(1){
		...
		if(mode < 8){//各種調整モードでなければ
			...
			while(get_sw() == 0){
				...
				if(get_IR(IR_L) - get_IR(IR_R) > 30){//左寄り
					led(8);
					
				}else if(get_IR(IR_L) - get_IR(IR_R) < -30){//右寄り
					led(1);
	...
	ESmotor(path,powor,non_stop,kabe);
  	...
  	if((non_stop == 0 || non_stop == 4) && (kabe == 1 || kabe == 4)){
   		// GyroSum_reset();
			...
			if(180 < get_IR(IR_F) ){//前壁　激突対策
				ESmotor(-25,powor,true,false);//ちょっと下がる
			}
...
//   int cnt2 = 0;
  ...
  while(1){
	...
	/*
	if(flag != 0 && get_IR(IR_F) >  F_max-20 ){//前壁が近すぎる場合は
		while(1){//前壁補正
	...
	if(abs((get_encoder_total_L() + get_encoder_total_R())/2 -  enc_base) > (h1+h1) ){
		led(9);
...
void S_run_kabe_naname(int powor, char flag, int LR){//壁切れまで走行
  ...
  if(Lflag == 3 && get_IR(IR_L) > 80){//壁切れが遅かった場合
	ESmotor(360,powor,true,false);///222
	
  }else if(Rflag == 3 && get_IR(IR_R) > 80){//壁切れが遅かった場合
	ESmotor(360,powor,true,false);///222
    ...
    switch(comand){
      ...
      case 0://S
	  	if(queue_empty()){
        	...
        	if(path_num == 1){
				...
				if(queue_next() < 0){//次　左
	          	  S_run_kabe(run_speed + run_fin_speed_offset,true,1);
				  //S_run_kabe(run_speed + run_fin_speed_offset,4,1);//non_stop = 4
				
			  	}else if(queue_next() > 0){//次　右
				  S_run_kabe(run_speed + run_fin_speed_offset,true,2);
				  //S_run_kabe(run_speed + run_fin_speed_offset,4,2);//non_stop = 4
			  	}else{
				  S_run_kabe(run_speed + run_fin_speed_offset,true,3);
				  //S_run_kabe(run_speed + run_fin_speed_offset,4,3);// w_flag = 4 串の壁補正あり
			  	}
				
				S_run(h1,run_speed + run_fin_speed_offset,4,true);//non_stop = 4
				...
				if(queue_next() < 0){//次　左
	          	  S_run_kabe(run_speed + run_fin_speed_offset,true,1);
				  //S_run_kabe(run_speed + run_fin_speed_offset,4,1);// w_flag = 4 串の壁補正あり
				
			  	}else if(queue_next() > 0){//次　右
				  S_run_kabe(run_speed + run_fin_speed_offset,true,2);
				  //S_run_kabe(run_speed + run_fin_speed_offset,4,2);// w_flag = 4 串の壁補正あり
			  	}else{
				  S_run_kabe(run_speed + run_fin_speed_offset,true,3);
				  //S_run_kabe(run_speed + run_fin_speed_offset,4,3);// w_flag = 4 串の壁補正あり
			  	}
	  ...
	  case 10://S 未確定の直線
        if(path_num == 1){
			...
			S_run_maze_search(path_num,run_speed + run_fin_speed_offset);//串のあり、なしは関数内で設定する
			
		}else{
			S_run_maze_search(path_num,run_speed + run_fin_speed_offset);
		}
    ...
    switch(comand){
      case -1://L
		...
		*/	
			...
			/*if(queue_next() == 0 && comand_old == 0){//直線、L、直線の時は大曲
      ...
      case 10://Snaname
	    ...
	    if(path_num <= 0){
			...
			if(run_fin_speed_offset > 0){//速度オフセットがプラスの時は無効化
				S_run(s45 /2 ,70,true,3); // w_flag = 3 斜めの壁補正あり 少しだけ前に移動した方が安全
			}else{
				S_run(s45 /2 ,70 + run_fin_speed_offset,true,3); // w_flag = 3 斜めの壁補正あり 少しだけ前に移動した方が安全
			}
			
		}else{
			if(run_fin_speed_offset > 0){//速度オフセットがプラスの時は無効化
				S_run(s45 * (long long)path_num + s45/2 ,70,true,3); // w_flag = 3 斜めの壁補正あり
			}else{
        ...
        break;
      case 1://R
		if(queue_next() == -1){//Sターン
			R_curve(sr90,true);
			ESmotor(200,35,true,true);//距離、スピード
			
		}else if(queue_next() == 1){//Uターン
			R_curve(sr90,true);
			ESmotor(250,35,true,true);//距離、スピード
		...
		*/	
			...
			/*if(queue_next() == 0 && comand_old == 0){//直線、R、直線の時は大曲
      ...
      case 11://R45
  		...
  		if(queue_next() == 11){//Vターン
			R_rotate_naname(r45 * path_num * 1.10,true);//0.75
