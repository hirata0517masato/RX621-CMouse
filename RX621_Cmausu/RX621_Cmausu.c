...
//    int nx,ny,tx,ty;
    ...
    int path_hosei[16] = {0,
 			  	...
 			  	200,200,200,200,200,200,200};//path_numごとに距離補正する
...
void run_shortest_path_fin(	char naname){
 ...
 /*     while(!queue_empty()){//debug
	...
	case 0://S
		...
		if(path_num >= 2){
			S_run((h1 * (long long)path_num-1) + path_hosei[path_num-1] ,run_speed + run_fin_speed_offset,4,4);//non_stop = 4 // w_flag = 4 串の壁補正あり
		}
