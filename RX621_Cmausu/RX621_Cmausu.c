...
void mae_kabe(){
	...
	while(t_1ms < F_max_time){//前壁補正
		...
		if(get_IR(IR_FL) - get_IR(IR_FR) > 10){
			motor(-F_pow, -F_pow/2);
			//motor(-F_pow, F_pow);
		...
		}else if(get_IR(IR_FL) - get_IR(IR_FR) < -10){
			motor(-F_pow/2, -F_pow);
			//motor(F_pow, -F_pow);
...
void S_run_kabe_naname2(int powor, char flag, int LR, int v2_flag){//壁切れまで走行
	...
	ESmotor(140,powor,true,false);  //140
...
void run_shortest_path_fin(	char naname){
    ...
    int path_hosei[31] = {0,
 			      0,0, 250,300, 400,400, 350,350, 400,400, 400,400, 350,350, 350,350, 
