...
void mae_kabe(){
	...
	while(t_1ms < F_max_time){//前壁補正
		...
		if(get_IR(IR_FL) - get_IR(IR_FR) > FLR_dif && get_IR(IR_FL) - get_IR(IR_FR) < FLR_dif_max){//左側が前壁に近すぎる
			motor(-F_pow,0);
			cnt = 0;
			GyroSum_reset();
			
		}else if(get_IR(IR_FL) - get_IR(IR_FR) < -FLR_dif && get_IR(IR_FL) - get_IR(IR_FR) > -FLR_dif_max){//右側が前壁に近すぎる
			motor(0,-F_pow);
			cnt = 0;
			GyroSum_reset();
...
void maze_search_adachi(short target_x,short target_y){
	...
	if(target_x == my_x && target_y == my_y){//ゴール
		...
		while(t_1ms < F_max_time || get_IR(IR_FL) < F_min){//スタートの奥まで進む 前壁補正
			...
			if(get_IR(IR_FL) - get_IR(IR_FR) > FLR_dif){//左側が前壁に近すぎる
				motor(-F_pow,0);
				cnt = 0;
				GyroSum_reset();
				
			}else if(get_IR(IR_FL) - get_IR(IR_FR) < -FLR_dif){//右側が前壁に近すぎる
				motor(0,-F_pow);
				cnt = 0;
				GyroSum_reset();
...
void maze_search_all(){
	...
	motor(0,0);
	/*	if(my_x == Get_Goal_x() && my_y == Get_Goal_y()){//最短経路上に未確定マスがなければ終了
		led_down();
		led_up();
		break;
		}
    ...
    */	
	...
	case -1://L
	    ...
	    */
		...
		L_curve(sl90,true);
		ESmotor(200,30,true,true);//距離、スピード 200,30
	...
	case 0://S
		...
		if((get_IR(IR_R) > 20 && get_IR(IR_R) < 90 ) || (get_IR(IR_L) > 20 && get_IR(IR_L) < 90 ) || get_IR(IR_L) > 200 || get_IR(IR_R) > 200 || ((abs(get_IR(IR_L) - get_IR(IR_R)) > 120) &&  get_IR(IR_L) > 20 && get_IR(IR_R) > 20)){//左右の差が大きい && 左右に壁がある
