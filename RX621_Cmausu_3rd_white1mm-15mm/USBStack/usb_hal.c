...
/*The actual Interrupt handlers are supplied by RPDL.
RPDL will call USBHALInterruptHandler.*/
#ifdef NOT_HANDLED_BY_RPDL
		  ...
		  (g_Bulk.m_OUT.m_BuffSize > g_Bulk.m_OUT.m_DataBuff.NumBytes))
	{
			...
			(g_Bulk.m_OUT.m_BuffSize >= (g_Bulk.m_OUT.m_DataBuff.NumBytes+2)))
		{
			...
			else
			{
				...
				volatile uint16_t dummy = USBIO.D1FIFO.WORD;
			}
		...
		else
		{
			...
			{
				/*Bulk out was set up to read data but throw it away*/
				volatile uint8_t dummy = (uint8_t)USBIO.D1FIFO.BYTE.L;	
			}
...
static void WriteBulkINPacket(void)
{
	...
	/*Select Bulk Pipe and 16 bit access*/
...
static void ReadControlOUTPacket(void)
{
	...
	(Count != CONTROL_OUT_PACKET_SIZE))
	{
		...
		//USBIO.BRDYENB.BIT.PIPE2BRDYE = 0;/////////////////////////////////////////////////////////bug?
...
static void HandleBRDY(void)
{
	...
	//if((1 == USBIO.BRDYSTS.BIT.PIPE2BRDY))
	{
...
static void HandleSetupCmd(void)
{
	/*Use type uint16_t to match registers*/
	uint16_t SetupCmdBuffer[USB_SETUP_PACKET_SIZE/2];
	uint16_t* p_16;
...
{	
	/*** BULK OUT ***/
	{
		...
		/*Pipe Configure, Direction, Type, BRDY Interrupt operation*/
		/*Default */ 
