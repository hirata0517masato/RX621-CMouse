 ...
 *
 * Copyright (C) 2009-2022 Renesas Electronics Corporation. All rights reserved.
 ******************************************************************************/
/******************************************************************************
 ...
 *         : 04.03.2014 2.50    Added support for RX21A and RX220. Deprecated
 *                              use of BLANK_CHECK_2_BYTE and BLANK_CHECK_8_BYTE
 *                              and now recommend use of BLANK_CHECK_SMALLEST.
 *                              Added FLASH_ERROR_OVERFLOW return code.
 *         : 14.02.2022 3.00    Changed configuration from one dependent on
 *                              r_bsp to one not dependent on r_bsp.
 *                              Corrected the mask value of DFLRE and DFLWE
 *                              register settings when RX610 is selected in
 *                              R_FlashDataAreaAccess function.
 *                              Fixed to return FLASH_ERROR_ADDRESS when the
 *                              block number of data flash is specified in
 *                              the argument in R_FlashProgramLockBit and
 *                              R_FlashReadLockBit function.
 *                              Fixed the problem that the global variable used
 *                              in FRDYI interrupt processing is initialized
 *                              after executing the FCU command with
 *                              the following functions when
 *                              the non-blocking mode is enabled.
 *                              - R_FlashErase
 *                              - R_FlashEraseRange
 *                              - R_FlashDataAreaBlankCheck
 *                              Fixed an the problem where the global variable 
 *                              (g_current_mode) initialized by FRDYI interrupt
 *                              processing is referenced after executing
 *                              the FCU command with the following functions
 *                              when non-blocking mode is enabled.
 *                              -R_FlashWrite
 *                              -R_FlashErase 
 *                              Fixed the problem that the exit_pe_mode function
 *                              could not clear the error status when writing
 *                              just before each ROM area boundary failed in
 *                              non-blocking mode, and changed the exit_pe_mode
 *                              function to a process that does not use
 *                              an argument.
 *                              Added clearing process for global variable
 *                              g_fcu_pclk_command in flash_reset function.
 *                              Changed infinite loop processing (while (1);)
 *                              to nop () processing.
 *                              Added IR flag clear processing for FRDYI
 *                              interrupt to exit_pe_mode function.
 ******************************************************************************/
...
#define WAIT_TRESW              (35*(FLASH_API_RX_CFG_ICLK_HZ/1000000))
/* The number of ICLK ticks needed for 10us delay are calculated below */
#define WAIT_T10USEC            (10*(FLASH_API_RX_CFG_ICLK_HZ/1000000))
 ...
 * The nice thing about using this implementation is that it does not take any extra RAM or ROM.
 */
...
/******************************************************************************
 Typedef definitions
 ******************************************************************************/
/* These typedefs are used for guaranteeing correct accesses to memory.
   When working with the FCU sometimes byte or word accesses are required. */
...
/* Used for holding data when DF to DF write is performed */
...
/* Flash initialization function prototype */
...
/* Used for checking that an input block is valid. */
static bool     flash_valid_block_check (uint32_t block);
 ...
 ******************************************************************************/
#endif /* FLASH_API_RX_CFG_COPY_CODE_BY_API */
...
static uint8_t data_flash_write(uint32_t address, uint32_t data, uint32_t size)
{
...
#endif
    {
        ...
        *(FCU_BYTE_PTR)(DF_ADDRESS) = 0xD0;
        ...
        {
#if defined(FLASH_API_RX_CFG_DATA_FLASH_BGO)
            /* If DF BGO is enabled then the flash ready interrupt may already
             * have fired by now if a DF access violation occurred. The ISR
             * would clear the error and take the FCU out of PE state. Another
             * DF AE violation could occur when the interrupt returns because
             * the code above is still attempting to write to DF. To clear this
             * error the FCU will need to be put back into PE state so that
             * we can perform a status clear.
             */
            if (FLASH_READY == g_flash_state)
            {
                /* The only reason the state would be FLASH_READY is if the
                 * interrupt had already fired.
                 */
                g_current_mode = FLD_PE_MODE;

                enter_pe_mode(address);
            }
#endif
            /* Return FLASH_FAILURE, operation failure */
            return FLASH_FAILURE;
 ...
 *                       RX210         support block and 2 byte checks
 * Arguments    : address -
 ...
 *                    If the parameter 'size'=='BLANK_CHECK_SMALLEST',
 *                    this should be set to an appropriate address boundary
 *                    based upon the smallest blank check size. The smallest
 *                    blank check size is the same as the smallest program
 *                    size for the data flash.
 ...
 *                    to either 'BLANK_CHECK_SMALLEST' or
 *                    'BLANK_CHECK_ENTIRE_BLOCK'. Use of 'BLANK_CHECK_8_BYTE'
 *                    and 'BLANK_CHECK_2_BYTE' has been deprecated but is
 ...
 *                FLASH_ERROR_ALIGNED -
 *                    Address not on programming boundary (only required when
...
uint8_t R_FlashDataAreaBlankCheck(uint32_t address, uint8_t size)
{
    ...
    if ((address >= BLOCK_DB0) && (address < (DF_NUM_BLOCKS + BLOCK_DB0)))
    {
        ...
        /* Data flash block sent in. */
        address = g_flash_BlockAddresses[address];
    }
    else if ((address >= DF_ADDRESS) &&
             (address < (DF_ADDRESS + FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES)))
    {
        /* Data flash address sent in. */
        if (BLANK_CHECK_SMALLEST == size)
        {
    ...
    {
        /* Neither a DF block or DF address were input. */
        return FLASH_ERROR_ADDRESS;
    }

    /* Check to make sure 'size' parameter is valid */
    if ((BLANK_CHECK_ENTIRE_BLOCK != size) && (BLANK_CHECK_SMALLEST != size))
    {
    ...
    if (FLASH_SUCCESS != flash_grab_state(FLASH_BLANKCHECK))
    {
    ...
    if (FLASH_SUCCESS != enter_pe_mode(address))
    {
    ...
    if (BLANK_CHECK_SMALLEST == size)
    {
        ...
        /* Disable FCU ready interrupt for smallest size checks */
        ...
        FLASH.DFLBCCNT.WORD = (uint16_t)(address & (DF_BLOCK_SIZE_LARGE - DF_PROGRAM_SIZE_SMALL));
    }
...
#ifdef FLASH_API_RX_CFG_DATA_FLASH_BGO
    /* Only using BGO on entire block blank checks */
    ...
    *ptrb = 0xD0;
    ...
    if (1 == FLASH.FSTATR0.BIT.ILGLERR) /* Check 'ILGERR' bit */
    {
...
{
#if defined(FLASH_MCU_RX610)

    /* Set Read access for the Data Flash blocks */
    FLASH.DFLRE.WORD = (uint16_t)(0x2D00 | (read_en_mask & 0x000F));

    /* Set Erase/Program access for the Data Flash blocks */
    FLASH.DFLWE.WORD = (uint16_t)(0x1E00 | (write_en_mask & 0x000F));
...
#if FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES == 8192
...
#if FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES > 16384
...
/******************************************************************************
 * Function Name: flash_valid_block_check
 * Description  : Checks to see if the input block is actually a valid block.
...
static bool flash_valid_block_check(uint32_t block)
{
    ...
    if (block >= BLOCK_DB0)
    {
        /* Data flash block sent in. Verify this block exists. */
        if (block >= (BLOCK_DB0 + DF_NUM_BLOCKS))
        {
            err = false;
        }
    }
...
/******************************************************************************
 End of function  flash_valid_block_check
 ******************************************************************************/
...
{
    /* Attempt to grab state */;
    if (FLASH_SUCCESS != flash_grab_state(FLASH_LOCK_BIT))
    {
        /* Another operation is already in progress */
        return FLASH_BUSY;
    }
...
void flash_ready_isr(void)
{
    ...
    if (FLASH_ERASING == g_flash_state)
    {
        /* Erase is done */
...
#ifdef FLASH_API_RX_CFG_DATA_FLASH_BGO
    else if (FLASH_BLANKCHECK == g_flash_state)
    {
...
#endif
    else if (FLASH_WRITING == g_flash_state)
    {
        ...
        if (g_bgo_bytes > 0)
        {
            ...
            if (1 == g_flash_to_flash_op)
            {
                ...
                if (FLASH_SUCCESS != enter_pe_mode(g_bgo_flash_addr))
                {
                    /* Leave Program/Erase Mode and clear any error flags */
                    exit_pe_mode();
...
/* If ROM programming is not enabled then do not compile this code as it is
    ...
    *(FCU_BYTE_PTR)address = 0xD0;
    ...
    {
#if defined(FLASH_API_RX_CFG_ROM_BGO)
        /* If ROM BGO is enabled then the flash ready interrupt may already
         * have fired by now if a ROM access violation occurred. The ISR
         * would clear the error and take the FCU out of PE state. Another
         * ROM AE violation could occur when the interrupt returns because
         * the code above is still attempting to write to ROM. To clear this
         * error the FCU will need to be put back into PE state so that
         * we can perform a status clear.
         */
        if (FLASH_READY == g_flash_state)
...
#else
    /* These macros are used to remove warnings from the compiler that the
     * parameters are never referenced. Using this macro will not actually
     * generate code.
     */
    INTERNAL_NOT_USED(address);
...
static uint8_t enter_pe_mode(uint32_t flash_addr)
{
    ...
    {
        /* Initialize the FCU, and store operation result in result variable */
        /* Check if FCU initialization was successful */
        if (FLASH_SUCCESS != flash_init())
        {
            /* FCU initialization failed - return operation failure */
            return FLASH_FAILURE;
        }
    }
    ...
    if (ROM_PE_MODE == g_current_mode)
    {
#if defined(FLASH_API_RX_CFG_ROM_BGO)
        /* Don't enable flash interrupts if programming lock bits */
        if (FLASH_LOCK_BIT != g_flash_state)
        {
    ...
    else if (FLD_PE_MODE == g_current_mode)
    {
#if defined(FLASH_API_RX_CFG_DATA_FLASH_BGO)
        /* Re-enable the FRDYI interrupt */
        FLASH.FRDYIE.BIT.FRDYIE = 1;
    ...
    if (0 == g_fcu_pclk_command)
    {
        ...
        if (0 != notify_peripheral_clock((FCU_BYTE_PTR) flash_addr))
        {
            /* Return FLASH_FAILURE, operation failure */
            return FLASH_FAILURE;
        }
...
static void exit_pe_mode(void)
{
        ...
        || (1 == FLASH.FSTATR0.BIT.PRGERR))
    {
        ...
        {
            /* Normally, this process does not run.
            If this function is called in read mode with an error condition,
            it sets this address and transitions to data flash P/E mode
            to send status clear command. */
    ...
    /* Clear FRDYI interrupt request flag */
    IR(FCU, FRDYI) = 0;
...
uint8_t R_FlashErase(uint32_t block)
{
    ...
    /*  Set which P / E mode is ROM or data flash */
#if defined(FLASH_API_RX_CFG_DATA_FLASH_BGO) || defined(FLASH_API_RX_CFG_ROM_BGO) || defined(DF_GROUPED_BLOCKS)
    ...
    /* Attempt to grab state */
    if (FLASH_SUCCESS != flash_grab_state(FLASH_ERASING))
    {
        /* Another operation is already in progress */
        return FLASH_BUSY;
    }
    ...
    if (FLASH_SUCCESS != enter_pe_mode(p_addr))
    {
        /* Make sure part is in ROM read mode. */
        exit_pe_mode();
    ...
    /* Only disable lock bit protection if user has specified to
       do so earlier */
    if (false == g_lock_bit_protection)
    {
        /* Cancel the ROM Protect feature */
        FLASH.FPROTR.WORD = 0x5501;
    }
 ...
 *                    Erase attempted to go past end of data flash.
 ******************************************************************************/
uint8_t R_FlashEraseRange(uint32_t start_addr, uint32_t bytes)
{
    ...
    if (start_addr >= DF_ADDRESS && start_addr < (DF_ADDRESS + FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES))
    {
        ...
        if ((start_addr + bytes) > (DF_ADDRESS + FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES))
        {
    ...
    /* Enter ROM PE mode, check if operation successful */
    if (FLASH_SUCCESS != enter_pe_mode(start_addr))
    {
        /* Make sure part is in ROM read mode. */
        exit_pe_mode();
...
static uint8_t flash_erase_command(FCU_BYTE_PTR const erase_addr)
{
...
#if defined(FLASH_API_RX_CFG_DATA_FLASH_BGO) || defined(FLASH_API_RX_CFG_ROM_BGO)
    /*  Set which P / E mode is ROM or data flash */
    uint8_t  current_mode = g_current_mode;
#endif
    ...
    *erase_addr = 0xD0;
    ...
    /* Check FCU error */
    if ((1 == FLASH.FSTATR0.BIT.ILGLERR) || (1 == FLASH.FSTATR0.BIT.ERSERR))
    {
 ...
 *                    Flash is busy with another operation
 *                FLASH_ERROR_OVERFLOW -
 *                    Write attempted to go past end of ROM or DF.
 ******************************************************************************/
                     ...
                     uint16_t bytes)
{
    ...
    if (flash_addr >= DF_ADDRESS && (flash_addr < DF_ADDRESS + FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES))
    {
        /* Check if the number of bytes were passed is a multiple of the
        ...
        /* Check to make sure erase will not go over the end of DF. */
        if ((flash_addr + bytes) > (DF_ADDRESS + FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES))
        {
            return FLASH_ERROR_OVERFLOW;
        }
    }
    /* Check for ROM area */
            ...
            && (flash_addr < ROM_PE_ADDR + FLASH_API_RX_CFG_ROM_SIZE_BYTES))
    {
        ...
        if ((flash_addr + bytes) > (ROM_PE_ADDR + FLASH_API_RX_CFG_ROM_SIZE_BYTES))
        {
            return FLASH_ERROR_OVERFLOW;
        }
#endif
    ...
    /* Attempt to grab state */
    if (FLASH_SUCCESS != flash_grab_state(FLASH_WRITING))
    {
        /* Another operation is already in progress */
        return FLASH_BUSY;
    }
    ...
    if ( (buffer_addr >= ROM_PE_ADDR) ||
        ((buffer_addr >= DF_ADDRESS) &&
         (buffer_addr < (DF_ADDRESS + FLASH_API_RX_CFG_DATA_FLASH_SIZE_BYTES))))
    {
        /* Buffering is needed. */
        /* Get maximum programming size that can currently be used. */
    ...
    if (FLASH_SUCCESS != enter_pe_mode(flash_addr))
    {
        /* Make sure part is in ROM read mode. */
        exit_pe_mode();
...
static uint8_t notify_peripheral_clock(FCU_BYTE_PTR flash_addr)
{
    ...
    FLASH.PCKAR.WORD = (FLASH_API_RX_CFG_FCLK_HZ/1000000);
...
static void flash_reset(void)
{
    ...
    /* The clock supplied to the flash needs to be notified to the FCU again. */
...
static uint32_t flash_get_program_size(uint32_t bytes, uint32_t flash_addr)
{
    ...
    /* These macros are used to remove warnings from the compiler that the
     * 'bytes' and 'flash_addr' parameters are never referenced. This happens
     * when a MCU does not have more than 1 DF program size. Using this macro
     * will not actually generate code.
     */
...
/******************************************************************************
 End of function  flash_get_program_size
 ******************************************************************************/
...
uint8_t R_FlashProgramLockBit(uint32_t block)
{
    ...
    /* Make sure valid block was input. */
    if ((false == flash_valid_block_check(block)) || (block >= BLOCK_DB0))
    {
        return FLASH_ERROR_ADDRESS;
    }
    ...
    /* Enter ROM PE mode, check if operation successful */
    if (FLASH_SUCCESS != enter_pe_mode((uint32_t) p_addr))
    {
        /* Make sure part is in ROM read mode. */
        exit_pe_mode();
